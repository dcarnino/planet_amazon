...
#                   Functions
#==============================================
def instantiate(n_classes, n_dense=1024, resnet_json="resnet50_128_mod.json", target_size=(197,197,3), verbose=1):
    """
    Instantiate the resnet 16.
    """
    ...
    model.compile(optimizer=Adam(lr=0.0008), loss="binary_crossentropy", metrics=[fbs])
...
def finetune(base_model, model, X_train, y_train, X_val, y_val,
             ...
             resnet_h5_1="resnet50_128_fine_tuned_1.h5",
             resnet_h5_check_point_1="resnet50_128_fine_tuned_check_point_1.h5",
             layer_names_file="resnet50_128_mod_layer_names.txt", verbose=1):
    """
...
def finetune_from_saved(resnet_h5_load_from, resnet_h5_save_to,
             resnet_json, X_train, y_train, X_val, y_val, nb_freeze=11,
             ...
             resnet_h5_check_point="resnet50_128_fine_tuned_check_point_2.h5", verbose=1):
    """
    Finetune the resnet 16 from already fine-tuned one.
    """
    ...
    loaded_model.load_weights(resnet_h5_load_from)
    ...
    loaded_model.fit_generator(
        ...
        callbacks=[EarlyStopping(monitor='val_loss', patience=patience),
                   ModelCheckpoint(filepath=resnet_h5_check_point, save_best_only=True),
                   ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=patience_lr)],
        class_weight=class_weight)

    # save weights
    loaded_model.save_weights(resnet_h5_save_to)
...
def train_for_a_fold(df_train, df_val, fold_id, target_size=(197,197),
                     model_dir="../data/planet_amazon/models/",
                     image_dir="../data/planet_amazon/train-jpg/",
                     verbose=1):
    """
    Train an ResNet50 for a fold.
    """
    ...
    ### Create model
    if verbose >= 1: print("\tInstantiating ResNet50 (fold %d)..."%fold_id)
    n_classes = y_train.shape[1]
    base_model, model = instantiate(n_classes, n_dense=1024, resnet_json=model_dir+"resnet50_128_mod_%d.json"%fold_id, verbose=verbose)

    ### Train model
    if verbose >= 1: print("\tFine-tuning ResNet50 first pass (fold %d)..."%fold_id)
    finetune(base_model, model, X_train, y_train, X_val, y_val, batch_size=40, epochs_1=3,
             ...
             resnet_h5_check_point_1=model_dir+"resnet50_128_fine_tuned_check_point_1_%d.h5"%fold_id,
             layer_names_file=model_dir+"resnet50_128_mod_layer_names.txt",
             verbose=verbose)
    del(base_model)
    del(model)
    K.clear_session()
    if verbose >= 1: print("\tFine-tuning ResNet50 second pass (fold %d)..."%fold_id)
    finetune_from_saved(model_dir+"resnet50_128_fine_tuned_check_point_1_%d.h5"%fold_id,
                        ...
                        X_train, y_train, X_val, y_val, batch_size=40, epochs=1000, optimizer_lr=0.0003,
                        nb_freeze=80, patience=4, patience_lr=1, class_imbalance=False,
                        nb_train_samples=len(y_train), nb_validation_samples=len(y_val),
                        resnet_h5_check_point=model_dir+"resnet50_128_fine_tuned_check_point_2_%d.h5"%fold_id,
                        verbose=verbose)
    K.clear_session()
    """if verbose >= 1: print("\tFine-tuning ResNet50 third pass (fold %d)..."%fold_id)
    finetune_from_saved(model_dir+"resnet50_128_fine_tuned_check_point_2_%d.h5"%fold_id,
                        ...
                        model_dir+"resnet50_128_mod_%d.json"%fold_id,
                        X_train, y_train, X_val, y_val, batch_size=40, optimizer_lr=0.00003,
