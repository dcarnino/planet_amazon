"""
    Name:           train_model.py
    Created:        10/7/2017
    Description:    Fine-tune vgg16_128 for Planet Amazon.
"""
#==============================================
#                   Modules
#==============================================
...
#==============================================
_EPSILON = K.epsilon()
from train_model_vgg16_128 import fbs, binary_crossentropy_weighted
...
def load_model(chosen_metrics=[fbs],
               vgg16_128_json="vgg16_128_mod.json",
               vgg16_128_h5="vgg16_128_fine_tuned_2.h5", verbose=1):
    """
    Load the vgg16_128 and trained weights from disk.
    """

    # load json and create model
    with open(vgg16_128_json, 'r') as iOF:
        loaded_model_json = iOF.read()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights(vgg16_128_h5)
...
def main(verbose=1):
    ...
    if sys.argv[2] != 'test':
        ...
        model = load_model(vgg16_128_json=model_dir+"vgg16_128_mod_%d.json"%fold_id,
                           vgg16_128_h5=model_dir+"vgg16_128_fine_tuned_check_point_3_%d.h5"%fold_id,
                           verbose=verbose)
        ...
        with open("../data/planet_amazon/vgg16_128_preds%d_%d.npy"%(fold_id,n_inference), "wb") as iOF:
            np.save(iOF, y_pred)
        with open("../data/planet_amazon/vgg16_128_trues%d_%d.npy"%(fold_id,n_inference), "wb") as iOF:
            np.save(iOF, y_test)
        ...
        with open("../data/planet_amazon/vgg16_128_predstest_%d.npy"%n_inference, "wb") as iOF:
            np.save(iOF, y_pred_folds)
        with open("../data/planet_amazon/vgg16_128_idstest_%d.txt"%n_inference, "w") as iOF:
