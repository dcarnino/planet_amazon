"""
    Name:           train_model.py
    Created:        10/7/2017
    Description:    Fine-tune resnet50 for Planet Amazon.
"""
#==============================================
#                   Modules
#==============================================
...
#==============================================
_EPSILON = K.epsilon()
from train_model_resnet50 import fbs, binary_crossentropy_weighted
...
def load_model(chosen_metrics=[fbs],
               resnet50_json="resnet50_mod.json",
               resnet50_h5="resnet50_fine_tuned_2.h5", verbose=1):
    """
    Load the resnet50 and trained weights from disk.
    """

    # load json and create model
    with open(resnet50_json, 'r') as iOF:
        loaded_model_json = iOF.read()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights(resnet50_h5)
...
def main(verbose=1):
    ...
    if sys.argv[2] != 'test':
        ...
        model = load_model(resnet50_json=model_dir+"resnet50_mod_%d.json"%fold_id,
                           resnet50_h5=model_dir+"resnet50_fine_tuned_check_point_2_%d.h5"%fold_id,
                           verbose=verbose)
        ...
        with open("../data/planet_amazon/resnet50_preds%d_%d.npy"%(fold_id,n_inference), "wb") as iOF:
            np.save(iOF, y_pred)
        with open("../data/planet_amazon/resnet50_trues%d_%d.npy"%(fold_id,n_inference), "wb") as iOF:
            np.save(iOF, y_test)
        ...
        with open("../data/planet_amazon/resnet50_predstest_%d.npy"%n_inference, "wb") as iOF:
            np.save(iOF, y_pred_folds)
        with open("../data/planet_amazon/resnet50_idstest_%d.txt"%n_inference, "w") as iOF:
