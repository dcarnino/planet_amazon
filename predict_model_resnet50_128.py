"""
    Name:           train_model.py
    Created:        10/7/2017
    Description:    Fine-tune resnet50_128 for Planet Amazon.
"""
#==============================================
#                   Modules
#==============================================
...
#==============================================
_EPSILON = K.epsilon()
from train_model_resnet50_128 import fbs, binary_crossentropy_weighted
...
def load_model(chosen_metrics=[fbs],
               resnet50_128_json="resnet50_128_mod.json",
               resnet50_128_h5="resnet50_128_fine_tuned_2.h5", verbose=1):
    """
    Load the resnet50_128 and trained weights from disk.
    """

    # load json and create model
    with open(resnet50_128_json, 'r') as iOF:
        loaded_model_json = iOF.read()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights(resnet50_128_h5)
...
def infer(model, X_test, y_test, X_train, n_inference=100, batch_size=32, verbose=1):
    """
    Infer with the resnet50_128.
    """
...
def main(verbose=1):
    ...
    if sys.argv[2] != 'test':
        ...
        model = load_model(resnet50_128_json=model_dir+"resnet50_128_mod_%d.json"%fold_id,
                           resnet50_128_h5=model_dir+"resnet50_128_fine_tuned_check_point_2_%d.h5"%fold_id,
                           verbose=verbose)

        ### Load images for val
        if verbose >= 1: print("Loading val images into RAM (fold %d)..."%fold_id)
        ...
        if verbose >= 1: print("Loading train images into RAM (fold %d)..."%fold_id)
        image_dir="../data/planet_amazon/train-jpg/"
        target_size = (256,256)
        df_train = pd.read_csv("../data/planet_amazon/train%d.csv"%fold_id)
        X_train, y_train = [], []
        for image_id, y_lab in tqdm(list(zip(df_train.image_name, df_train.iloc[:,2:].values)), miniters=100):
            image_path = image_dir+str(image_id)+".jpg"
            if os.path.exists(image_path):
                try:
                    img = load_img(image_path, target_size=target_size)
        ...
        n_inference = 20
        y_pred, y_test = infer(model, X_val, y_val, X_train, n_inference=n_inference, batch_size=32, verbose=verbose)
        if verbose >= 2:
            print(y_pred.shape)
            print(y_test.shape)
            print(y_pred[0,0,0])
        ...
        with open("../data/planet_amazon/resnet50_128_preds%d_%d.npy"%(fold_id,n_inference), "wb") as iOF:
            np.save(iOF, y_pred)
        with open("../data/planet_amazon/resnet50_128_trues%d_%d.npy"%(fold_id,n_inference), "wb") as iOF:
            np.save(iOF, y_test)

    else:

        ### Load images for test
        if verbose >= 1: print("Loading test images into RAM...")
        ...
        with open("../data/planet_amazon/resnet50_128_predstest_%d.npy"%n_inference, "wb") as iOF:
            np.save(iOF, y_pred_folds)
        with open("../data/planet_amazon/resnet50_128_idstest_%d.txt"%n_inference, "w") as iOF:
